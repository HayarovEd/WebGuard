...
import org.squareup.okhttp.internal.huc.CacheAdapter;
import org.squareup.okhttp.internal.huc.HttpURLConnectionImpl;
import org.squareup.okhttp.internal.huc.HttpsURLConnectionImpl;
...
public final class OkUrlFactory implements URLStreamHandlerFactory, Cloneable {
  ...
  public HttpURLConnection open(URL url) {
    ...
    return open(url, client.getProxy(), null);
  }

  public HttpURLConnection open(URL url, String hostIp) {
    return open(url, client.getProxy(), hostIp);
  }

  HttpURLConnection open(URL url, Proxy proxy) {
    return open(url, proxy, null);
  }

  //HttpURLConnection open(URL url, Proxy proxy) {
  HttpURLConnection open(URL url, Proxy proxy, String hostIp) {
    String protocol = url.getProtocol();
    OkHttpClient copy = client.copyWithDefaults();
    copy.setProxy(proxy);
    if (hostIp != null)
      copy.setHostIp(hostIp);
