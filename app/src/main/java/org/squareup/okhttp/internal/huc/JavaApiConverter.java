 ...
 */
package org.squareup.okhttp.internal.huc;

import org.squareup.okhttp.Handshake;
import org.squareup.okhttp.Headers;
import org.squareup.okhttp.MediaType;
import org.squareup.okhttp.Request;
import org.squareup.okhttp.Response;
import org.squareup.okhttp.ResponseBody;
import org.squareup.okhttp.internal.Util;
import org.squareup.okhttp.internal.http.OkHeaders;
import org.squareup.okhttp.internal.http.StatusLine;
...
import org.squareup.okio.BufferedSource;
import org.squareup.okio.Okio;
...
public final class JavaApiConverter {
   ...
   * See {@link HttpURLConnection#getRequestProperties()} for details.
   */
  ...
  /**
   * Creates a {@link CacheResponse} of the correct (sub)type using information
  ...
  /**
   * Creates an {@link HttpURLConnection} of the correct subclass from the supplied OkHttp
   * {@link Response}.
   */
  ...
  /**
   * Extracts an immutable request header map from the supplied {@link org.squareup.okhttp.Headers}.
   */
   ...
   * extracted. See {@link #extractStatusLine(CacheResponse)}.
   */
  ...
  /**
   * Extracts OkHttp headers from the supplied {@link HttpURLConnection}. Only real headers
   * are extracted. See {@link #extractStatusLine(HttpURLConnection)}.
   */
  ...
  /**
   * Extracts the status line from the supplied Java API {@link HttpURLConnection}.
  ...
  /**
   * Extracts the status line from the supplied Java API {@link CacheResponse}.
   * As per the spec, the status line is held as the header with the null key. Returns {@code null}
   * if there is no status line.
   */
  ...
  private static final class CacheHttpsURLConnection extends DelegatingHttpsURLConnection {
    ...
    /*@Override*/ public long getContentLengthLong() {
      //return delegate.getContentLengthLong(); // TODO XXX
      return (int) delegate.getContentLength();
    }
    ...
    /*@Override*/ public long getHeaderFieldLong(String field, long defaultValue) {
      //return delegate.getHeaderFieldLong(field, defaultValue); // TODO XXX
      return (long) delegate.getHeaderFieldInt(field, (int) defaultValue);
    }
  }
