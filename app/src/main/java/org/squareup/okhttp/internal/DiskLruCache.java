...
package org.squareup.okhttp.internal;
...
public final class DiskLruCache implements Closeable {
  ...
  private void readJournalLine(String line) throws IOException {
    ...
    if (secondSpace >= 0 && firstSpace == CLEAN.length() && line.startsWith(CLEAN)) {
      String[] parts = line.substring(secondSpace + 1).split(" ");
      entry.readable = true;
      entry.currentEditor = null;
      entry.setLengths(parts);
    } else if (secondSpace < 0 && firstSpace == DIRTY.length() && line.startsWith(DIRTY)) {
      entry.currentEditor = new Editor(entry);
    } else if (secondSpace < 0 && firstSpace == READ.length() && line.startsWith(READ)) {
