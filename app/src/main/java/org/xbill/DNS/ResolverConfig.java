...
ResolverConfig() {
	...
	if (servers == null || searchlist == null) {
		...
		if (OS.indexOf("Windows") >= 0) {
			...
			findNetware();
		} else if (vendor.indexOf("Android") >= 0) {
			findAndroid();
		} else {
			findUnix();
		}
		...
		while ((line = br.readLine()) != null) {
			...
			if (line.indexOf(host_name) >= 0) {
				...
				if (name.labels() == 1)
					continue;
				addSearch(s, lsearch);
			} else if (line.indexOf(primary_dns_suffix) >= 0) {
				while (st.hasMoreTokens())
					s = st.nextToken();
				if (s.equals(":"))
					continue;
