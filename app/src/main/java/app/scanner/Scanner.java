...
public class Scanner
{
	...
	public static final String proDBDomain	 = "full." + Settings.APP_SITE; // "full.webguard.app"
	...
	/* ----------------------------------------------------------- */
/*
	public static void scan (MemoryBuffer buffer, TCPStateMachine tcpStateMachine)
	{
//		  L.d(Settings.TAG_SCANNER, "Scanning buffer of size = " + buffer.size());

//		  buffer.rewind();
//		  byte[] buf = buffer.readAll();
//		  L.e(Settings.TAG_SCANNER, new String(buf));
//		  tcpStateMachine.sendBuffer(buffer);

		//sendText("<html><head><title>BLOCKED</title></head><body><h1>Открытие страницы заблокировано антивирусом!</h1><br/>Данный ресурс является зловредным, или распространяет зловредные программы.</body><html/>", tcpStateMachine);
		//tcpStateMachine.sendText("<html><head><title>BLOCKED</title></head><body><h1>Открытие страницы заблокировано антивирусом!</h1><br/>Данный ресурс является зловредным, или распространяет зловредные программы.</body><html/>");
	}
*/
/*
	private static void sendText (String text, TCPStateMachine machine)
	{
		MemoryBuffer buf = MemoryCache.alloc(0);

		byte[] data = text.getBytes();
		String header = "HTTP/1.1 200 OK\r\nContent-Type: text/html; charset=utf-8\r\nContent-Length: " + data.length + "\r\n\r\n";
		buf.write(header.getBytes());
		buf.write(data);
		buf.rewind();

		machine.sendBuffer(buf);

		MemoryCache.release(buf);
	}
*/
	/* ----------------------------------------------------------- */
	 ...
	 */
	public int scanDomain (String url) { return scanDomainInternal(url, null, true, true); }
	public int scanDomain (String url, String referer) { return scanDomainInternal(url, referer, true, true); }

	//public int scanDomainFast (String url) { return scanDomainInternal(url, null, false, true); }
...
//		  if (url.indexOf("test3.com") >= 0)
//			  useSigs = (useSigs) ? true : false;
		...
		{
			//char c = domain1_s[l - 1].charAt(0); // check for ip
			//if (c >= '0' && c <= '9')
	 ...
	 * TODO XXX, optimize
	 */
	public ScanResult scanUrl (String url) { return scanUrl(url, null); }
