...
//	  public static final String patcherName = "libbspatch.so";
										...
										int patchFlags)
	{
		...
		/*
		// prepare
		String runPath = activity.getApplicationInfo().nativeLibraryDir;
		runPath += (runPath.endsWith("/")) ? LibPatch.patcherName : "/" + LibPatch.patcherName;

		File runFile = new File(runPath);
		if (!runFile.canExecute())
		{
			L.e(Settings.TAG_LIBPATCH, "invalid executable '" + runPath + "'");
		}
		else
		{
			// usage: bspatch oldfile newfile patchfile
			ProcessBuilder pb = new ProcessBuilder(runPath, oldFile, newFile, patchFile);
			...
			if (!retval)
			{
				String cmd = TextUtils.join(", ", pb.command());
				L.e(Settings.TAG_LIBPATCH, "execute error (" + cmd + ")");
			}
		}
		*/
