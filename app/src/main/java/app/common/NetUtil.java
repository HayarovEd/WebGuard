...
public class NetUtil
{
	...
	public static boolean isSharingWiFi(final WifiManager manager)
	{
		...
		{
			Statistics.addLog(Settings.LOG_SHARING_WIFI_ERR);
			ex.printStackTrace();
	...
	public static boolean hasInternetPermission()
	{
		...
		try
		{
			DatagramSocket socket = new DatagramSocket(56783);
			socket.connect(new InetSocketAddress(Settings.APP_SITE, 53));
			socket.send(new DatagramPacket("TEST".getBytes(), 4));
			socket.close();
		}

		catch (SocketException e) { mes = e.getMessage(); e.printStackTrace(); }
		catch (IOException e) { mes = e.getMessage(); e.printStackTrace(); }
	...
	public static Record[] lookupNS(String domain) {
		...
		// bug fix - google dns (8.8.8.8 и 8.8.4.4) не отдает список authoritative name server https://developers.google.com/speed/public-dns/
	...
	public static String lookupIp(String domain)
	{
		...
		try
		{
			...
			if (records != null)
			{
				//InetAddress[] ar = new InetAddress[records.length];
