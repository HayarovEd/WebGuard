...
public class Firewall
{
	...
	private static HashSet<String> mobileBlocked = new HashSet<String>(350);
	private static HashSet<String> wifiBlocked	 = new HashSet<String>(350);
	private static boolean isWifiNetwork = false;

	// TODO XXX use BitSet
	private static final Set<Integer> mobileAllowedUidCache = new HashSet<Integer>(); // cache for allowed apps in mobile network
	private static final Set<Integer> mobileBlockedUidCache = new HashSet<Integer>(); // cache for blocked apps in mobile network
	private static final Set<Integer> wifiAllowedUidCache	= new HashSet<Integer>(); // cache for allowed apps in wifi network
	private static final Set<Integer> wifiBlockedUidCache	= new HashSet<Integer>(); // cache for blocked apps in wifi network

	private static boolean inited = false;
	private static String firewallFileName = null;
	...
	private static void load()
	{
		synchronized (lock)
		{
			try
			{
				...
				if (str != null)
				{
					HashSet <String> mobileBlocked = new HashSet<String>(350);
					HashSet <String> wifiBlocked = new HashSet<String>(350);
	...
	public static void save()
	{
		synchronized (lock)
		{
			...
			if (obj != null)
			{
				...
				try { Utils.saveFile(data, firewallFileName); } catch (IOException e) { e.printStackTrace(); }
			}
		}
	}
	...
	public static void mobileAppState(String pkgName, boolean allow)
	{
		...
		if (!allow && !isWifiNetwork)
			FilterVpnService.notifyDropConnections(App.getContext()); // update blocked apps for mobile network
	}
	...
	public static void wifiAppState(String pkgName, boolean allow)
	{
		...
		if (!allow && isWifiNetwork)
			FilterVpnService.notifyDropConnections(App.getContext()); // update blocked apps for wifi network
	}
