...
//			  else if (LibNative.asciiStartsWith("Connection: ", line))
//			  {
//				  if (Policy.changeUserAgentAndIp)
//					  res.headers.set(i, "Connection: close");
//			  }
...
//			  int dataStart = Utils.getFileStart(data, 0, size);
//			  if (dataStart > 0 && dataStart < size)
//				  res.requestData = Arrays.copyOfRange(data, dataStart, size);
		}
	}
...
//			bug fix from Roman Popov 29.04.2017 (don't worked on Android 7)
//			if ((ch < ' ' || ch > 0x127) && ch != '\r' && ch != '\n' && ch != 0x22)
//			if ((ch < ' ' || ch > 0x127) && ch != '\r' && ch != '\n' && ch != 0x16)

//			if ((ch < ' ' || ch > 0x127) && ch != '\r' && ch != '\n') {
//				if (Settings.DEBUG) L.e("R", "char: " + (int) ch);
//				break;
//			}
...
/*	  public static RequestHeader getConnectRequest(String domain, int port)
	{
		RequestHeader res = new RequestHeader();
		res.headers = new ArrayList<String>(1);
		res.headers.add("CONNECT "+domain+":"+port+" HTTP/1.1\r\n\r\n");
		res.modified = true;
		res.parsedAsHttp = true;
	...
	public int getRequestWithData(byte[] buf)
	{
		...
		if (requestData != null)
		{
			//byte[] buf = new byte[hh.length + requestData.length];
