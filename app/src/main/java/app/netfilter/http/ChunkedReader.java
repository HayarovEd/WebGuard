...
public class ChunkedReader
{
	...
	public boolean read(byte[] buf, int offset, int size)
	{
		...
		while (true)
		{
			...
			// если строка завершена \r\n
			if (LibNative.asciiEndsWith("\r\n", s))
			{
				if (Settings.DEBUG) L.d(Settings.TAG_CHUNKEDREADER, "line: ", s);

				// смещаемся на длину прочитанного, а если часть была прочитана раньше, то отнимаем длину старого
				...
				if (s.length() > 2)
				{
					...
					catch (NumberFormatException e)
					{
						if (Settings.EVENTS_LOG)
							Statistics.addLog(Settings.LOG_CHUNKPARSE_NULL_ERR + s.trim().split(";")[0]);
			...
			{
				// прочитали кусок строки, окончание будет в следующем пакете
				lastLineStart = s;
