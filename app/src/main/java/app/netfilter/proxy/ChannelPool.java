...
public class ChannelPool
{
								...
								byte[] key1, byte[] key2, int key1Len, int key2Len)
	{
		...
		synchronized (pool)
		{
			...
			if (size < CAPACITY)
			{
				...
				//if (isBad(data, System.currentTimeMillis(), 0L)) // initial ping
				//{
				//	  if (DEBUG) L.d(Settings.TAG_CHANNELPOOL, "Proxy connect reuse failed");
				//}
				//else
				{
					// ping ok
		...
		synchronized (pool) {
			...
			while (it.hasNext()) {
				...
				if (!onlyBad || data.poolAddTime < timeoutAdd || isBad(data, time, timeoutPing)) {
					...
					try { data.channel.close(); } catch (IOException e) { e.printStackTrace(); }
					data.channel = null;
	...
	{
		/*
		boolean haveData = (fd != 0) ? LibNative.socketHaveDataToSend(fd) : false;
		if (haveData)
			return 0;

		boolean sendOk = (fd != 0) ? LibNative.socketSendDataForce(fd, pingData) : true;
		if (!sendOk)
			return -1;
		*/
		...
		//haveData = (fd != 0) ? LibNative.socketHaveDataToSend(fd) : false;
		//if (!sendOk || haveData)
		if (!sendOk)
			return -1;
