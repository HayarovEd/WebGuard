...
public class UDPClient implements IClient
{
	...
	//private final ClientEvent ev = new ClientEvent();
						...
						byte[] localIp, int localPort)
	{
		...
		this.pkgs = null; // все равно оно не находится
	...
	public boolean onSockEvent(SelectionKey selKey)
	{
		...
		try
		{
			if (selKey.isValid() && selKey.isReadable())
			{
				...
				try
				{
					synchronized (vpn)
					{
						if (chan != null)
						{
							...
							if (addr != null)
							{
								if (Settings.DEBUG) L.d(Settings.TAG_UDPCLIENT, "Answer from: ", addr.toString());
				...
				{
					if (Settings.DEBUG) L.i(Settings.TAG_UDPCLIENT, "onSockEvent: read error ", e.toString());
						...
						if (!resp.parsed)
						{
							...
							if (resp.readNameError && request != null)
							{
								if (Settings.EVENTS_LOG)
									Statistics.addLog(Settings.LOG_UDPCLIENT_DNSPARSE_ERR + request.toString());
							}
						}
	...
	public boolean onTunInput(Packet packet)
	{
		...
		if (Settings.DEBUG_NET) L.a(Settings.TAG_UDPCLIENT, "UDPPacket length = ", Integer.toString(packet.length));
...
//				  int remaining = writeBuffer.remaining();
				...
				if (packet.dstPort == 53 && dnsAddresses != null && dnsAddresses.size() > 0)
				{
					...
					for (int i = 0; i < size; i++)
					{
						...
						// if webguard is updated while connected to wifi it gets local dns as 1.1.1.1
			...
			{
				if (Settings.DEBUG) L.e(Settings.TAG_UDPCLIENT, "onTunInput: failed to write packet to server ", e.toString());

				final String mes = e.getMessage();
				if (mes != null && (mes.indexOf("EACCES") >= 0 || mes.indexOf("EPERM") >= 0))
					((IFilterVpnApplication) App.getContext()).onPermissionDenied();
