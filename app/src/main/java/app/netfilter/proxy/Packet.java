...
//	  public static native int addIpTcpHeaderNative(byte[] r0, byte[] r1, int i2, byte[] r3, int i4, int i5,
//													  int i6, int i7, int i8, int i9);
//	  public static native short calcCheckSum(int startValue, byte[] buf, int offset, int length);
	...
	/*
	public static short calcCheckSum_(int paramInt1, byte[] paramArrayOfByte, int paramInt2, int paramInt3)
	{
		int i = paramInt2 + (paramInt3 & 0xFFFFFFFE);
		for (int j = paramInt2; j < i; j += 2)
			paramInt1 += ((0xFF & paramArrayOfByte[j]) << 8 | 0xFF & paramArrayOfByte[(j + 1)]);
		if ((paramInt3 & 0x1) != 0)
			paramInt1 += ((0xFF & paramArrayOfByte[j]) << 8);
		int k = (0xFFFF & paramInt1 >> 16) + (paramInt1 & 0xFFFF);
		return (short)(0xFFFFFFFF ^ k + (0xFFFF & k >> 16));
	}
	*/
	...
	/*
	public static short calcTcpCheckSum(byte[] r0, int i0, int i1, byte[] r1, byte[] r2)
	{
		int startValue = 0 + (i1 + 6) +
						(r1[0] & 0xFF << 8 | r1[1] & 0xFF) +
						(r1[2] & 0xFF << 8 | r1[3] & 0xFF) +
						(r2[0] & 0xFF << 8 | r2[1] & 0xFF) +
						(r2[2] & 0xFF << 8 | r2[3] & 0xFF);
		return Packet.calcCheckSum(startValue, r0, i0, i1);
	}
	*/
	...
	/*
	private static short calcUdpCheckSum(byte[] r0, int i0, int i1, byte[] r1, byte[] r2)
	{
		int startValue = 0 + (i1 + 17) +
						(r1[0] & 0xFF << 8 | r1[1] & 0xFF) +
						(r1[2] & 0xFF << 8 | r1[3] & 0xFF) +
						(r2[0] & 0xFF << 8 | r2[1] & 0xFF) +
						(r2[2] & 0xFF << 8 | r2[3] & 0xFF);
