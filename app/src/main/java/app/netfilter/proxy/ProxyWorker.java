...
{
	private ProxyEvent ev; //	private final ProxyEvent ev;

	static final int TUN_WRITE_PACKET_SIZE = 2048;
		...
		if ((packet.flag & Packet.TCP_FLAG_SYN) == 0)
		{
			...
			if (client == null)
			{
				...
				if ((packet.flag & Packet.TCP_FLAG_FIN) != 0)
				{
					...
					//if ((packet.totalLen - packet.dataOfs) > 0)
						// проверил даже в smali, никаких действий с этим нет
				}
...
//		int uid = NetInfo.findMatchingUidInTcp(packet.srcPort, packet.dstIp, packet.dstPort); // Roman Popov
	...
	private void processTunReadPacketUDP(Packet packet)
	{
		...
		while (client == null)
		{
			...
			int uid = 0; //NetInfo.findMatchingUidInUdp(packet.srcPort, packet.dstIp, packet.dstPort);
			DNSRequest req = null;
			//PolicyRules rules = (policy != null) ? policy.getPolicy(packet) : new PolicyRules();

			//if (uid != 0)
			//	  if (DEBUG) L.e(Settings.TAG_PROXYWORKER, "Found uid for UDP: " + uid);
			...
			if (policy != null)
			{
				...
				if (packet.dstPort == 53)
				{
					...
					//if (req.parsed)
					//	  // TODO XXX overwrite packet policy with DNS policy
					//	  rules = policy.getPolicy(req);		// check DNS request
				}
			...
			// TODO сделать добавление домена в клиент, чтобы поймать при эксепшене в DNS-ответе
	...
	private void networkProcessingThread()
	{
		while (true)
		{
			...
			//selectedKeys = selector.selectedKeys(); // old variant
			...
			// TODO XXX f*ck!!! check time and update proxy servers on every iteration
			//if (policy != null)
				//policy.update(System.currentTimeMillis());
	...
	private void dataWait()
	{
		...
		catch (ClosedSelectorException e)
		{
			e.printStackTrace();
			// TODO XXX ???
			FilterVpnService.notifyConfigChanged(App.getContext(), FilterVpnService.ACTION_CHANGED);
		}
	...
	//public void setTimeout(IClient client, long timeout)
	{
	...
	public void stop()
	{
		...
		if (pfd != null)
			try { pfd.close(); } catch (IllegalStateException ie) { } catch (IOException e) { /*e.printStackTrace();*/ }

		//LibNative.signalSendSelf(1);
		if (tunReadThread != null)
