...
{
	//protected final ClientEvent ev; // Roman Popov: why final?
	...
	//private boolean isShutdownSent = false;
	...
	//private PolicyRules curPolicyRules = new PolicyRules();
...
//	  private ResponseHeader response = null;
	...
	// NOT THREAD SAFE!!! (now not used =)) size x2 for any modifications
	...
	private TCPStateMachine(TCPClient tcpClient, byte[] servIp, int servPort, byte[] locIp, int locPort, int ack)
	{
		...
		//timeout = TIMEOUT_RECEIVE_INITIAL;
	...
	*/
	...
	// check policy and start using proxy if needed
/*
	public void useProxy()
	{
		if (this.policy != null)
		{
			proxied = policy.isProxyUse(serverIp, serverPort, client.getUid(), client.isBrowser());
			if (proxied) //&& servPort != policy.getProxyPort(); // RRR для теста отключил проксирование этого же порта
				proxyConnectState = PROXY_STATE_INITED;
		}
	}
*/
	...
	{
		//return (isShutdownSent) && (isFinAckReceivedFromApp);
	...
	private void setClosingState()
	{
		...
		try { client.sockEnableReadEvent(false); } catch (IOException e) { e.printStackTrace(); }
	}
	...
	private static void cipherInit(final PublicKey publicKey)
	{
		...
		catch (NoSuchAlgorithmException ex) { ex.printStackTrace(); cipher = null; }
		catch (NoSuchPaddingException ex) { ex.printStackTrace(); cipher = null; }
		catch (InvalidKeyException ex) { ex.printStackTrace(); cipher = null; }
	}
...
//		try {
		try {
			...
			//cipher.doFinal(input, 0, inputLen, input, inputLen);

			//error:04000071:RSA routines:OPENSSL_internal:DATA_TOO_LARGE, 512
			//пофиксил передавая 256 байтный массив
...
//			System.out.println("TEST cipherDoFinal 6");
//			reinit = true;
//		}
   ...
   private boolean proxyHello_v01()
			...
			if (read > 0)
			{
				...
				if (pos >= 28 && CHECK_BUFFER[0] == 'Y' && CHECK_BUFFER[1] == 'O')
				{
					...
					else if (CHECK_BUFFER[2] == 'R' && CHECK_BUFFER[3] == 'D')
					{
					   // redirect
						byte[] ip = new byte[4];
						ip[0] = CHECK_BUFFER[4]; ip[1] = CHECK_BUFFER[5]; ip[2] = CHECK_BUFFER[6]; ip[3] = CHECK_BUFFER[7];
						byte[] token = new byte[8];
						token[0] = CHECK_BUFFER[20]; token[1] = CHECK_BUFFER[21]; token[2] = CHECK_BUFFER[22]; token[3] = CHECK_BUFFER[23];
						token[4] = CHECK_BUFFER[24]; token[5] = CHECK_BUFFER[25]; token[6] = CHECK_BUFFER[26]; token[7] = CHECK_BUFFER[27];
						// TODO redirect
					}
				}
			}
	...
	// Рома добавил метод для генерации секретного байта
	...
	private boolean proxyAuth_v01()
	{
		...
		do {
			...
			CHECK_BUFFER_SEC[4] = 0; CHECK_BUFFER_SEC[5] = 0; CHECK_BUFFER_SEC[6] = 0; CHECK_BUFFER_SEC[7] = (byte) Policy.getProxyFlags(); // flags1
			CHECK_BUFFER_SEC[8] = 0; CHECK_BUFFER_SEC[9] = 0; CHECK_BUFFER_SEC[10] = 0; CHECK_BUFFER_SEC[11] = 0; // flags2
			...
			// crypt buffer
			int t = 0;
//			while (buf.position() == 0 && cipher != null && t < 4)
			// увеличил с 4 до 10 попыток
			while (buf.position() == 0 && cipher != null && t < 10) {

				t++;
//				byte b = random.generateByte();
//				if (b == 0 || (b & 0xFF) >= 0xC0) {
//					System.out.println("TEST proxyAuth 7.1");
//					continue;
//				}
...
//			if (cipher == null || t >= 4)
			// увеличил с 4 до 10 попыток
			if (cipher == null || t >= 10) {
				// тут жопа настала и не смогли авторизоваться на сервере - сообщаем об этом если можем
				Statistics.addLog(Settings.LOG_ERROR_AUTH_ON_PROXY);
				break;
			}
		...
		if (!ok) {
			Statistics.addLog(Settings.LOG_ERROR_AUTH_ON_PROXY2);
			proxySetConnectState(PROXY_STATE_ERROR);
			return false;
		}
	...
	private boolean proxyAuth_v02()
	{
		...
		do
		{
			...
			CHECK_BUFFER[14] = 0; CHECK_BUFFER[15] = 0; CHECK_BUFFER[16] = 0; CHECK_BUFFER[17] = (byte) Policy.getProxyFlags();
			CHECK_BUFFER[18] = 0; CHECK_BUFFER[19] = 0; CHECK_BUFFER[20] = 0; CHECK_BUFFER[21] = 0;

			if (isGPId)
				CHECK_BUFFER[54] = 0; CHECK_BUFFER[55] = 0; CHECK_BUFFER[56] = 0; CHECK_BUFFER[57] = 0; // reset id end
	...
	public boolean onSockConnected()
	{
		...
		try { thisId = random.generateInt(); } catch (SecurityException ex) { thisId = 7; }
		setSeqNo(thisId);
...
//			  debugging = (1==1);
		...
		try
		{
			...
			if (pos <= 0)
			{
				...
				//isShutdownSent = true;
				//ready = isReadyToClose();
				//if (!ready)
				//{
				//	  if (DEBUG_PROFILE_NET) ev.sockShutdown++;
				//	  if (!proxied)
				//		  client.sockWriteShutdown();
				//}
				//else
				//{
				//	  if (DEBUG_PROFILE_NET) ev.closeGracefully++;
	...
	private boolean onProxySockReady(boolean isWrite)
	{
		...
		case PROXY_STATE_UNAUTHORIZED:
			//if (!isWrite) return true; // nothing to do on read
			//break;
			if (isWrite) break;
	...
	public boolean onTimeout(long currentTime, int fd)
	{
			...
			timeout >= TIMEOUT_EXPIRE_LIMIT)
		{
			...
			do
			{
				if (state != State.HALF_CLOSED_BY_SERVER)
				{
					if (proxied || client.serverKeepAliveEnabled())
					{
						if (Settings.DEBUG_NET)
							if (!proxied) L.a(Settings.TAG_TCPSTATEMACHINE, "stop keepalive test (" + fd + ") (" + Integer.toHexString(thisId) + ")");
				...
				if (Settings.DEBUG_NET) L.a(Settings.TAG_TCPSTATEMACHINE, "timeout expired (" + fd + ")");
						...
						currentTime - proxyStateChangeTime >= TIMEOUT_STATE_LIMIT))
		{
			...
			// убрали потому что лочится весь трафик
//			if (proxied && client.curProxy != null) {
//				client.curProxy.setDead(true);
//			}
		...
		else
		{
			...
			//if (!(curPolicyRules == Policy.SCAN && !scanDataBuffered))
			{
	...
	private boolean checkAndWriteBuffer(ByteBuffer buffer)
	{
		...
		do
		{
			//if (serverPort == 443 || serverPort == 5223 || serverPort == 5228 || serverPort == 4244
			//	  break;

			int ppos = buffer.position(); // должна быть равна 0
			int dataSize = buffer.remaining();

			//if (dataSize == 276) // clash of clans initial packet size == 276
			//	  ready = true;
			...
			//int hasIviData = 0; // TODO XXX temporary (1 - config, 2 - video)
			...
			while ((req = RequestHeader.parse(buffer, serverIp, client.isBrowser(), client.getUid(), proxied, policy)) != null)
			{
				...
				// Костыль для Opera Classic
				// при включении режима сжатия она передаёт referrer только первый раз
				...
				PolicyRules rules = policy.getPolicy(req, client.getUid(), client.getPkg0(), client.isBrowser());
				...
				if (!wait && !drop)
				{
					...
					//hasIviData = Policy.iviCheckUrl(req);
					//if (hasIviData != 0)
					//	  hasDrop = true; // TODO XXX pipelining? other progs, not ivi?
				}
				...
				// это случай, при котором в конце буфера неполный запрос
				// мы его читаем в спец-буфер, потом допишем в конец
				if (wait)
				{
					if (Settings.DEBUG) L.w(Settings.TAG_TCPSTATEMACHINE, Integer.toString(requests.size()), " requests in WAIT");

//					  if (requestsInBuffer == 0)
					{
						buffer.clear();
...
//					  buffer.position(nextReqPos);
//					  partialRequestSize = p - nextReqPos;
//					  partialRequest = new byte[partialRequestSize];
//					  buffer.get(partialRequest, 0, partialRequestSize);
//					  String s = new String(partialRequest, 0, 0, partialRequestSize);
//					  L.e(Settings.TAG_TCPSTATEMACHINE, "Partial request:\n", s);
				}
				else
				{
					// все остальные запросы мы сохраняем в список, потом они пойдут по порядку в сокет-буфер
				...
				// при парсинге следующего запроса мы будем знать, позицию, откуда начался парсинг
				nextReqPos = buffer.position();
				requestsInBuffer++;

				if (Settings.DEBUG_HTTP) L.a(Settings.TAG_TCPSTATEMACHINE, "Request: ", req.toString());
			} // while ((req
			...
			if (Settings.DEBUG_HTTP) L.a(Settings.TAG_TCPSTATEMACHINE, "Sending ", Integer.toString(requests.size()), " requests");
			...
			if (hasDrop && requests.size() == 1)
			{
				...
				sockWriteBuf.clear();
//				  //sockWriteBuf.limit(dataSize);
//				  sockWriteBuf.position(dataSize);
				requests.clear();

				// send 404, we have 3 variants:
				// without connection close
				// with connection close without fin. in tcp: < data > ack > fin < ack > fin (normal close sequence)
				// with connection close with fin. in tcp: < data > ack < fin > fin (simultaneous close sequence)
			...
			if (requestsSize > 0)
			{
				for (int i = startRequestCount; i < requestsSize; i++)
				{
					...
					if (Settings.DEBUG_HTTP) L.a(Settings.TAG_TCPSTATEMACHINE, "Writing request to sockWriteBuf:\n", r.getHeaders());
		...
		if (requestsSize != 0) // ==-1 on binary data or >0 if need to send http data
		{
			try
			{
				...
				if (written < size)
					// по идее, так быть не может, все данные должны были уйти
	...
	private int checkIncomingData(byte[] data, int offset, int size)
	{
		...
		// запросов не было, пришла неведомая куйня, либо это что-то кодированное
		if (requests.size() == 0)
			return size;
		...
		if (req.loadedLength == 0 && req.response == null)
		{
			...
			// только пошел ответ на один из запросов
			if (req.policyRules.hasPolicy(PolicyRules.DROP))
			{
				// запрос не отправлялся на сервер, отправляем браузеру 404 и игнорим запрос
			...
			else if (!resp.full)
			{
				...
				//String s = new String(data, offset, size);
				return 0;
			}
			...
			if (Settings.DEBUG) L.w(Settings.TAG_TCPSTATEMACHINE, "ResponseHeader: ", resp.toString());

			// TODO XXX and what about else? (see below)
			req.response = resp;
			PolicyRules rules = policy.getPolicy(req, resp, data, client.getUid(),
													client.isBrowser(), proxied);
			req.policyRules = rules;
			...
			if (!resp.chunked)
			{
				...
				{
					if (Settings.DEBUG) L.w(Settings.TAG_TCPSTATEMACHINE, "Data loaded fully for ", req.toString());

					requests.remove(0);
					//if (proxied)
					//	  client.updateTimeout(10000); // need to update timeout?
				}
				...
				{
					if (Settings.DEBUG) L.e(Settings.TAG_TCPSTATEMACHINE, "We have next response in this packet!");
			...
			else if (dataSize > 0)
			{
				...
				if (chunkedReader.isAtEndOfFile())
				{
					...
					{
						if (Settings.DEBUG) L.e(Settings.TAG_TCPSTATEMACHINE, "We have next response in this packet! (after chunked)");
			...
			{
				// пришел только заголовок, но chunked
				requests.remove(0);
				return size;
			}
		}
		else
		{
			...
			{
				if (Settings.DEBUG) L.d(Settings.TAG_TCPSTATEMACHINE, "Continuation of chunked response");
	...
	private void hashData(RequestHeader request, ResponseHeader response, byte[] data, int offset, int size)
	{
		...
		{
			//long hasherSize = hasher.getSize();
			//L.d("TCP", "Hasher size = "+hasherSize);
...
//			  debugging = (1==1);
		...
		do
		{
			...
			if (!isSeqnoMatch(packet))
			{
				...
				if (Settings.DEBUG) L.e(Settings.TAG_TCPSTATEMACHINE, "Invalid seqNo! Sending ack! in ", Integer.toHexString(thisId));
			...
			else
			{
				...
				{
					try { client.sockEnableWriteEvent(true); } catch (IOException e) { e.printStackTrace(); }
					//client.sockForceEvents();
				}
			...
			if ((packet.flag & Packet.TCP_FLAG_FIN) != 0)
			{
				...
				try { client.sockEnableWriteEvent(has); } catch (IOException e) { e.printStackTrace(); }
			...
			//if (packet.flag != Packet.TCP_FLAG_ACK)
			//sendAck();
		}
		while (false);
...
/*
	// NOT USED, maybe invalid
	public void sendBuffer(MemoryBuffer buffer)
	{
		int bufSize = buffer.size();
		//L.d("TCP", "Sending buffer of " + bufSize + " bytes");
		int offset = 0;
		int seq = seqNo;
		int size = 0;
		//int pos = sockReadBuf.position();
		...
		while (bufSize > 0)
		{
			Packet packet = PacketPool.alloc(TCP_HEADER_LENGTH + bufSize);
			size = Math.min(bufSize, packet.frame.length - TCP_HEADER_LENGTH);
			buffer.read(packet.frame, TCP_HEADER_LENGTH, size);
//			  if (sockReadBuf.remaining() < size)
//				  sockReadBuf.clear();
//			  sockReadBuf.put(packet.frame, 40, size);

			//L.d("TCP", "sending data, seqNo = " + Integer.toHexString(seq));
			sendData(seq, sockWriteBuf.remaining(), packet, size);
			bufSize -= size;
			//offset += size;
			seq += size;
		}
		...
		//L.d("TCP", "Buffer sent, last seqNo = " + Integer.toHexString(seqNo));
		//client.sockClose();
		sockReadBuf.compact();
		//sendFin();
	}
...
/*
	...
	public void sendText(String text)
	{
		...
		if (Settings.DEBUG) L.d(Settings.TAG_TCPSTATEMACHINE, "Sending header buffer of ", Integer.toString(headerBytes.length), " bytes");

		sendData(seqNo + pos, sockWriteBuf.remaining(), packet, headerBytes.length);

		int seq = seqNo + headerBytes.length;
		int offset = 0;
		while (dataLength > 0)
		{
			packet = PacketPool.alloc(TCP_HEADER_LENGTH + dataLength);
			int size = Math.min(dataLength, packet.frame.length - TCP_HEADER_LENGTH);
			System.arraycopy(data, offset, packet.frame, TCP_HEADER_LENGTH, size);

			if (Settings.DEBUG) L.d(Settings.TAG_TCPSTATEMACHINE, "Sending text buffer of ", Integer.toString(size), " bytes");

			sendData(seqNo + pos + offset, sockWriteBuf.remaining(), packet, size);
...
*/
	...
	{
		if (Settings.DEBUG) L.d(Settings.TAG_TCPSTATEMACHINE, "send404 in ", Integer.toHexString(thisId));

		//String content = "<html><body>&nbsp;</body></html>";
		//String content = "<html><body><center><br/><br/><h1><font color='#ff0000'>W</font>ebGuard заблокировал вредоносную страницу!</h1></center></body></html>";
		String content = null;
		if (client.isBrowser())
		{
...
/*
	// TODO XXX rewrite this to use sendText or sendBuffer
	public void sendIviConfig()
	{
		L.a(Settings.TAG_TCPSTATEMACHINE, "sendIviConfig");
		if (Settings.DEBUG) L.d(Settings.TAG_TCPSTATEMACHINE, "sendIviConfig in ", Integer.toHexString(thisId));

		String content = Policy.iviGetConfig();
		if (content == null)
			return;
		...
		//if (DEBUG) L.d("TCP", "Sending iviConfig header buffer of " + headerBytes.length + " bytes");
...
//		  L.d(Settings.TAG_TCPSTATEMACHINE, "sendAck " + windowSize);
	...
	private boolean sendRetransmit()
	{
		...
		//if (pos > 0 && (scanDataBufferSize == 0 || isFinSent))
