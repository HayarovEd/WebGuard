...
public class TCPClient implements IClient
{
	...
	//private DeferredTask timeoutTask = null;
	private int uid;
	...
	int key1Len = 0;
	int key2Len = 0;
	//protected final byte[] testKey = {0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39};

	ProxyBase.ProxyServer curProxy = null; // active proxy
//	private int curProxyConnectTryCount = 0;
						...
						byte[] servIp, int servPort, byte[] locIp, int locPort, int ack)
	{
		...
		if (sm.hasBuffers())
		{
			//pkgs = Utils.getPackagesForUid(uid);
			...
			{
				if (Settings.DEBUG) L.e(Settings.TAG_TCPCLIENT, "NO Packages for uid: ", Integer.toString(uid));
			}
	...
	private boolean onConnectEvent(SelectionKey selectionKey)
	{
		...
		try
		{
			...
			if (curProxy != null)
			{
				// убрали потому что блочится весь трафик
				//curProxy.setDead(false); // clear dead count
		...
		catch (IOException e)
		{
			...
			try { chan.close(); } catch (IOException ex) { } // need or not?
			chan = null;

			// убрали потому что лочится весь трафик
//			if (curProxyConnectTryCount > 3)
//			{
//				L.e(Settings.TAG_TCPCLIENT, "Reconnecting to another server...");
//				curProxy.setDead(true);
//				curProxyConnectTryCount = 0;
//			}

			try
			{
				// убрали потому что лочится весь трафик
//				if (Settings.DEBUG) L.e(Settings.TAG_TCPCLIENT, "Server connect try " + curProxyConnectTryCount);
	...
	public boolean onTimeout(long currentTime)
	{
		...
		//if (fd != 0)
		//{
		//	  //L.d(Settings.TAG_TCPCLIENT, "fd " + fd);
		//	  //LibNative.socketPrintInfo(fd);
		//}
	...
	public synchronized boolean sockClose(boolean force)
	{
		try
		{
			//boolean alive = (chan.isConnected() || chan.isConnectionPending());
//			  if (chan != null)
//				  L.d(Settings.TAG_TCPCLIENT, "isOpen = ", Boolean.toString(chan.isOpen()));
	...
	public synchronized void sockConnect() throws IOException
	{
		...
		if (policy != null && proxify && proxy_ready)
		{
			...
			if (Settings.DEBUG) if (proxy == null) L.d(Settings.TAG_TCPCLIENT, "getProxyHost failed");
		}
		if (proxy != null)
		{
	...
	private void sockUpdateEvents() throws IOException
	{
		...
		if (chan != null)
		{
			...
			{
				Statistics.addLog(Settings.LOG_TCPCLIENT_CHANREG_ERR);
			}
		}
	}
	...
	public int sockRead(ByteBuffer buf, boolean skipCrypt) throws IOException
	{
		...
		if (read <= 0)
		{
			if (Settings.DEBUG) L.i(Settings.TAG_TCPCLIENT, "Read ", Integer.toString(read), " bytes!");
			return read;
		}
	...
	public int sockWrite(ByteBuffer buf, boolean skipCrypt) throws IOException
	{
		...
		{
			//buf.limit(buf.position() + Math.min(toWrite, 32768));
			//buf.limit(buf.position() + Math.min(toWrite, 4096));
	...
	protected boolean cryptBuf(ByteBuffer buf, int pos, int size, boolean isDecrypt)
	{
		...
		//int ml = size;//message.length; // TODO XXX must be long
		//int kl = testKey.length;
