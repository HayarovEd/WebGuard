...
public class TunWriteThread extends Thread
{
	...
	public void run()
	{
		...
		while (true)
		{
			...
			while (!exception && packets != null)
			{
				...
				while (!exception && (packet != null || (packet = packets.pollFirst()) != null))
				{
					...
					//	  L.w(Settings.TAG_TUNWRITETHREAD, "Packet: " + packet);
				...
				// next
				synchronized (writeQueue) { packets = writeQueue.pollFirst(); }
			}
		...
		try { out.close(); } catch (IOException e) { /*if (!interrupted) e.printStackTrace();*/ }
		worker = null;
